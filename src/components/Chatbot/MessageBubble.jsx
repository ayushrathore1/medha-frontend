...
const MessageBubble = ({ sender, text, isTyping, speed, onChar, onDone }) => (
  <div
    className={`flex mb-3 ${sender === "user" ? "justify-end" : "justify-start"}`}
  >
    {sender === "ai" && (
    ...
    <div
      className={`px-4 py-2 rounded-2xl max-w-[75%] text-base shadow transition-all ${
        sender === "user"
    ...
    {sender === "user" && (
      <span className="ml-2 flex items-center justify-center h-8 w-8 rounded-full bg-blue-500 text-white font-bold shadow">
        🧑
      </span>
    )}
    <style>
